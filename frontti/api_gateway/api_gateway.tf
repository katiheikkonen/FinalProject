...
resource "aws_api_gateway_resource" "userdata_id" {
  rest_api_id = aws_api_gateway_rest_api.mystocksapi.id
  parent_id   = aws_api_gateway_resource.userdata.id #  API Gatewayn pääpolun määrittely
  path_part   = "{user_id}" # API Gatewayn alapolku jonka alle metodi laitetaan, tässä tapauksessa DynamoDB:n taulun nimi
}
