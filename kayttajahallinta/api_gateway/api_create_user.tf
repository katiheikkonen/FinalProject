...
resource "aws_api_gateway_integration" "create_user" {
  ...
  type                    = "AWS"#"AWS_PROXY" #  This type of integration lets an API method be integrated with the Lambda function invocation action
  uri                     = aws_lambda_function.create_user.invoke_arn#module.lambda.create_user_lambda_arn #  API gateway invocation URI Lambda-funktiolle
}
...
resource "aws_lambda_permission" "create_user" {
  ...
  function_name = aws_lambda_function.create_user.function_name #module.lambda.create_user_lambda_name
  principal = "apigateway.amazonaws.com"

  source_arn = "${aws_api_gateway_rest_api.mystocksapi.execution_arn}/*/POST/userdata"
}