...
resource "aws_api_gateway_integration" "update_user" {
  ...
  #uri                     = module.lambda.create_user_lambda_invoke_arn #  API gateway invocation URI Lambda-funktiolle
}
...
resource "aws_lambda_permission" "update_user" {
  ...
  function_name = module.lambda_outputs.update_user_lambda_name
